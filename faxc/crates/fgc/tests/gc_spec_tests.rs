...
use fgc::{FgcConfig, FgcError, GarbageCollector};
use std::sync::Arc;
...
fn spec_allocate_zero_size_object() {
    ...
    match (addr1, addr2) {
        (Ok(a1), Ok(a2)) => {
            assert_ne!(a1, a2, "Zero-size allocations must return unique addresses")
        },
...
fn spec_allocate_maximum_size_object() {
    ...
    let config = FgcConfig {
        min_heap_size: 1 * 1024 * 1024,
        initial_heap_size: 1 * 1024 * 1024,
        max_heap_size: 2 * 1024 * 1024,
        soft_max_heap_size: 2 * 1024 * 1024,
        ..Default::default()
    };
    let gc = GarbageCollector::new(config).expect("Failed to create GC");
    ...
    assert!(
        ...
        "Large allocation should return OutOfMemory, got: {:?}",
        result
    );
}
...
fn spec_allocate_unaligned_size() {
    ...
    for size in [1, 7, 13, 17, 100, 127, 255] {
        ...
        assert_eq!(
            addr % 8,
            0,
            "Allocation of {} bytes not 8-byte aligned: {:#x}",
            size,
            addr
        );
    }
...
fn spec_concurrent_allocation_thread_safety() {
    ...
    let config = FgcConfig {
        min_heap_size: 100 * 1024 * 1024,
        initial_heap_size: 100 * 1024 * 1024,
        max_heap_size: 200 * 1024 * 1024,
        soft_max_heap_size: 200 * 1024 * 1024,
        ..Default::default()
    };
    ...
    for handle in handles {
        ...
        assert_eq!(
            unique_count,
            addrs.len(),
            "Thread {} got duplicate addresses",
            thread_id
        );
    ...
    let unique_count = all_addresses
        .iter()
...
fn spec_preserve_reachable_object() {
    ...
    assert_eq!(
        value, 0x12345678DEADBEEF,
        "Reachable object's data was corrupted by GC!"
    );
...
fn spec_transitive_reachability() {
    ...
    gc.unregister_root(root_addr)
        .expect("Failed to unregister root");
}
...
fn spec_register_null_root() {
    ...
    assert!(
        ...
        "Register null root should not panic"
    );
}
...
fn spec_root_update_after_relocation() {
    ...
    let config = FgcConfig {
        min_heap_size: 8 * 1024,
        initial_heap_size: 8 * 1024,
        max_heap_size: 8 * 1024,
        soft_max_heap_size: 8 * 1024,
        ..Default::default()
    };
    let gc = GarbageCollector::new(config).expect("Failed to create GC");
...
fn spec_duplicate_root_registration() {
    ...
    assert!(
        result.is_ok() || result.is_err(),
        "Duplicate registration should not panic"
    );
...
fn spec_unaligned_root_address() {
    ...
    assert!(
        result.is_ok() || result.is_err(),
        "Unaligned root should not panic"
    );
}
...
fn spec_allocation_overflow_protection() {
    ...
    assert!(
        matches!(
            result,
            Err(FgcError::OutOfMemory { .. }) | Err(FgcError::InvalidArgument(_))
        ),
        "Overflow allocation should fail gracefully, got: {:?}",
        result
    );
}
...
fn spec_stress_many_objects() {
    ...
    let config = FgcConfig {
        min_heap_size: 50 * 1024 * 1024,
        initial_heap_size: 50 * 1024 * 1024,
        max_heap_size: 100 * 1024 * 1024,
        soft_max_heap_size: 100 * 1024 * 1024,
        ..Default::default()
    };
    let gc = GarbageCollector::new(config).expect("Failed to create GC");
...
fn spec_stress_varying_sizes() {
    ...
    let sizes = [
        1, 7, 8, 13, 64, 127, 128, 255, 256, 512, 1023, 1024, 4096, 8192,
    ];
...
fn spec_stress_large_objects() {
    ...
    let config = FgcConfig {
        ...
        soft_max_heap_size: 20 * 1024 * 1024,
        ..Default::default()
    };
    let gc = GarbageCollector::new(config).expect("Failed to create GC");
