...
fn test_full_gc_cycle() {
    ...
    assert_eq!(
        fixture.state(),
        GcState::Idle,
        "GC should be in Idle state after completion"
    );
    assert!(
        fixture.cycle_count() > 0,
...
fn test_multiple_gc_cycles() {
    ...
    for i in 0..3 {
        ...
        assert!(
            ...
            "GC cycle {} should have completed",
            i + 1
        );
    }
}
...
fn test_gc_cycle_state_transitions() {
    ...
    assert_eq!(
        fixture.state(),
        GcState::Idle,
        "State should return to Idle after GC"
    );
}
...
fn test_gc_thread_pool_creation() {
    ...
    let heap =
        Arc::new(fgc::heap::Heap::new(config.clone()).expect("Failed to create heap for test"));
...
fn test_marker_root_scanning() {
    ...
    let _handle = marker
        .root_scanner()
        .register_global_root(&root_value as *const usize as usize, Some("test_root"));
...
fn test_marker_concurrent_marking_integration() {
    ...
    marker
        .start_concurrent_marking(2)
        .expect("Concurrent marking should start");
